...
// AJOUT : récupère l'élément qui affiche le nombre de résultats (S13)
...
function displayItems(items) {
    ...
    // AJOUT : met à jour le compteur de résultats (S13)
    // Si tous les animés sont affichés, on montre le total
    // Sinon on montre "X résultats sur Y"
    const total = data.length;
    if (items.length === total) {
        resultCount.textContent = `${total} animé${total > 1 ? "s" : ""}`;
    } else {
        resultCount.textContent = `${items.length} résultat${items.length > 1 ? "s" : ""} sur ${total}`;
    }
    ...
    items.forEach(item => {
        ...
        html += `
            <article class="card" data-id="${item.id}">
                <img src="${item.image}" alt="${item.name}" data-fallback="${encodeURIComponent(item.name)}">
                <div class="card-body">
                    <!-- AJOUT : badge coloré selon le thème (S13) -->
                    <span class="badge badge-${item.theme}">${item.theme}</span>
                    <h2>${item.name}</h2>
                    <p>${item.year}</p>
...
// AJOUT : gestion des images manquantes en JS pur (évite l'attribut onerror obsolète)
// Pour chaque image, on écoute l'événement "error" et on remplace par un placeholder
container.querySelectorAll("img").forEach(img => {
    img.addEventListener("error", () => {
        img.src = `https://placehold.co/400x300/1a1a2e/9090b0?text=${img.dataset.fallback}`;
    });
});
...
form.addEventListener("submit", (event) => {
    ...
    const newItem = {
        ...
        image: `https://placehold.co/400x300/1a1a2e/9090b0?text=${encodeURIComponent(name)}`
    };
